...
----------------------------------------------------------------- Default weapons for your NPC.
	['DefaultWeapons'] 			= { "weapon_pistol", "weapon_smg1"}, 
...
----------------------------------------------------------------- Increase or decrease NPC's self damage. The lowest is -100 (%) and it can go as high as you wish (be reasonable).
	['DamageSelfScale'] 		= 200, --%, 0 is default.
...
----------------------------------------------------------------- Custom functions.	An order doesn't matter. They are based on hooks.
	['Functions'] = {
		...
		local downchat = {
			"One down!",
			"Suspect expired.",
			"Overwatch, we got one.",
			"Suspect is down!",
			"Haha.",
			"One expired.",
			"Another one down.",
			"Don't resist.",
			"This is what happens when you don't resist."
		}
		NPCChatAdv("Civil Protection:", "<::"..downchat[math.random(1, #downchat)].."::>", 300, self)
		end,
		...
		['OnNPCTakeDamage'] = function(self, dmginfo)
			...
			"I got shot!",
			"Im bleeding!",
			"Im running low on vitals!",
			"Shit!",
			"Shit, Shit, that hurts!"
		}
		
            if math.random(1, 4) == 2 then NPCChatAdv("Civil Protection:", "<::"..atkchat[math.random(1, #atkchat)].."::>", 300, self) end
		...
		['OnNPCScaleDamageOnNPC'] = function(npc, hitgroup, dmginfo)	
			...
			"Suspect is hurt.",
			"Suspect is wounded.",
			"Suspect seems to be bleeding",
			"Its dying!", 
			"Death is imminent for the suspect.",
			"My aim SUCKS!",
			"He's bleeding!",
			"Suspect is loosing blood!"
		}
		if math.random(1, 10) == 2 && npc:Health() > npc:GetMaxHealth() / 2 then
		local self = dmginfo:GetAttacker()
		NPCChatAdvRand("Civil Protection:", atkchat, 300, self, true)
		elseif  math.random(1, 7) == 2 && npc:Health() < npc:GetMaxHealth() / 2 then
		local self = dmginfo:GetAttacker()
		NPCChatAdvRand("Civil Protection:", atk2chat, 300, self, true)
		end
		end,
		...
		if math.random(1, 10) == 2 && ply:Health() > 40 then
		local self = dmginfo:GetAttacker()
		NPCChatAdvRand("Civil Protection:", atkchat, 300, self, true)
		elseif  math.random(1, 7) == 2 && ply:Health() < 40 then
		local self = dmginfo:GetAttacker()
		NPCChatAdvRand("Civil Protection:", atk2chat, 300, self, true)
		end
		end,
